...
///
/// Loose port of stringMatch from places
#[inline]
...
// from https://searchfox.org/mozilla-central/source/xpcom/string/nsUTF8Utils.h
#[inline(always)]
...
fn next_codepoint_lower(s: &str) -> (char, usize) {
    ...
    // Note that it would be safe to use get_unchecked in these,
    // since we know `s` is a `str`, and all `str` must be
    // valid utf8, but I'm not daring enough for that :p
    let (uc, len) = unsafe {
        if is_2byte(c) {
